# MindBloom — AI Agent Working Instructions

This document defines how AI coding agents should understand, architect, and implement MindBloom.
...
MindBloom is a calm space for student wellness, combining meditation, mini-games, tracking, and an AI tree assistant.
...
User inputs intent
↓
User writes a note about their thoughts to grow their gratitude tree.
↓
User chats with the ancient tree spirit for reflection.
↓
User plays calming games to relieve immediate stress.
↓
...
- Do not use display or tall fonts or very small `max-w-*` values for body copy. Paragraphs should use the body font and a readable measure, generally around 45 to 75 characters per line.
- Do not hardcode UI surface, text, border, or semantic state colors in components. Use the semantic tokens so light and dark themes stay coherent.
- Use the Inter font token for body copy, headings, buttons, forms, and navigation. Use the tall or display face only for deliberate editorial hero typography.
